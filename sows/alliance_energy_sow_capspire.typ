#import "../templates/capspire_sow.typ": *
...
#let alliance_mobile_revival_data = (
  ...
  pricing: (
    ...
    // Using a simplified structure that matches the hour-based approach
    // The template no longer uses this data structure directly but we keep it for compatibility
    milestone_costs: (
      (milestone: "Mobile App Revival & Update", amount: [2,400], is_total: false),
      (milestone: "Total Estimated Cost", total_number: 2400, amount: [2,400], is_total: true)
    ),
  ...
  approval: (
    number: 9
    // Content handled by the template
  )
