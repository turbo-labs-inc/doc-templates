#let cover_page(
  ...
  grid(
    ...
    align(right)[#image("../images/turbo-black.png", width: 6cm)] // Adjust logo width
  )
...
#let section_timeline(number, dates, milestones) = {
    v(1cm)
...
#let section_pricing_resources(
    ...
    [
      #table(
        ...
        ..milestone_costs.map(m => (
          ...
          if m.is_total { 
            text(size: 14pt, fill: white, weight: "medium")[\$#m.amount]
          } else {
            text(weight: "regular")[\$#m.amount]
          }
        )).flatten()
      )
    ]
  ...
  let total_cost = milestone_costs.filter(m => m.is_total).at(0).total_number
...
subtitle,
  ...
  if "deliverables" in sow_content {
    ...
    section_deliverables_table(
      deliverables.number,
       deliverables.deliverables // The key inside deliverables data is also 'deliverables'
    )
  }
